...
namespace Microsoft.Playwright.Tests
{
    ...
    public class PageWaitForSelector2Tests : PlaywrightSharpPageBaseTest
    {
        ...
        public async Task ShouldNotConsiderVisibleWhenZeroSized()
        {
            ...
            var exception = await Assert.ThrowsAnyAsync<TimeoutException>(() => Page.WaitForSelectorAsync("div", new PageWaitForSelectorOptions { Timeout = 1000 }));
            Assert.Contains("Timeout 1000ms", exception.Message);
            await Page.EvaluateAsync("() => document.querySelector('div').style.width = '10px'");
            exception = await Assert.ThrowsAnyAsync<TimeoutException>(() => Page.WaitForSelectorAsync("div", new PageWaitForSelectorOptions { Timeout = 1000 }));
            Assert.Contains("Timeout 1000ms", exception.Message);
