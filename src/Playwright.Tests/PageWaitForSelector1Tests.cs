...
namespace Microsoft.Playwright.Tests
{
    ...
    public class PageWaitForSelector1Tests : PageTestEx
    {
        ...
        public async Task ElementHandleWaitForSelectorShouldTimeout()
        {
            await Page.SetContentAsync("<div></div>");
            var div = await Page.QuerySelectorAsync("div");
            var exception = Assert.ThrowsAsync<TimeoutException>(async () => await div.WaitForSelectorAsync("span", new ElementHandleWaitForSelectorOptions { State = WaitForSelectorState.Attached, Timeout = 100 }));
        ...
        public async Task ShouldReportLogsWhileWaitingForVisible()
        {
            ...
            var exception = Assert.ThrowsAsync<TimeoutException>(async () => await watchdog);
