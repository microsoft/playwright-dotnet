...
namespace Microsoft.Playwright.Tests
{
    ...
    public class PageSelectOptionTests : PageTestEx
    {
        ...
        public async Task ShouldNotSelectSingleOptionWhenSomeAttributesDoNotMatch()
        {
            ...
            Assert.ThrowsAsync<TimeoutException>(async () => await Page.SelectOptionAsync("select", new SelectOptionValue { Value = "green", Label = "Brown" }, new PageSelectOptionOptions { Timeout = 300 }));
        ...
        {
            await Page.GotoAsync(Server.Prefix + "/input/select.html");
            var exception = Assert.ThrowsAsync<PlaywrightException>(async () => await Page.SelectOptionAsync("body", string.Empty));
