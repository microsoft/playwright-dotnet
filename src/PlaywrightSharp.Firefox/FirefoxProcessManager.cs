...
{
    internal class FirefoxProcessManager : IProcessManager
    {
        ...
        /// <summary>
        /// Finalizes an instance of the <see cref="FirefoxProcessManager"/> class.
        /// </summary>
        ...
        /// <summary>
        /// Represents state machine for Firefox process instances. The happy path runs along the
        ...
        private abstract class State
        {
            ...
            /// <returns>A <see cref="Task"/> that completes when the start action is done.</returns>
            public virtual Task StartAsync(FirefoxProcessManager p) => Task.FromException(InvalidOperation("start"));
            ...
            /// <param name="timeout">The maximum waiting time for a graceful process exit.</param>
            /// <returns>A <see cref="Task"/> that completes when the exit action is done.</returns>
            public virtual Task ExitAsync(FirefoxProcessManager p, TimeSpan timeout) => Task.FromException(InvalidOperation("exit"));
            ...
            /// <param name="p">The Firefox process.</param>
            /// <returns>A <see cref="Task"/> that completes when the kill action is done.</returns>
            public virtual Task KillAsync(FirefoxProcessManager p) => Task.FromException(InvalidOperation("kill"));
            ...
            public virtual Task WaitForExitAsync(FirefoxProcessManager p) => p._exitCompletionSource.Task;

            /// <summary>
            /// Handles disposal of process and temporary user directory.
            /// </summary>
            /// <param name="p">Process.</param>
            public virtual void Dispose(FirefoxProcessManager p) => Disposed.EnterFrom(p, this);
            ...
            private class StartedState : State
            {
                ...
                public override Task StartAsync(FirefoxProcessManager p) => Task.CompletedTask;
            ...
            private class DisposedState : State
            {
                ...
                public override Task StartAsync(FirefoxProcessManager p) => throw new ObjectDisposedException(p.ToString());
